// 2026 US state income tax rates (flat effective rate approximations)
export const STATE_RATES: Record<string, number> = {
  ...
  HI: 0.082, ID: 0.058, IL: 0.0495,IN: 0.0305,IA: 0.06,
...
export function stateRate(stateCode: string): number {
  return STATE_RATES[stateCode.toUpperCase()] ?? 0.05;
}

// Federal effective rate approximation by annual net profit
export function federalEffectiveRate(annualProfit: number): number {
  if (annualProfit < 40_000)  return 0.10;
  if (annualProfit < 80_000)  return 0.14;
  if (annualProfit < 120_000) return 0.18;
  if (annualProfit < 180_000) return 0.22;
  if (annualProfit < 250_000) return 0.26;
  return 0.30;
}
